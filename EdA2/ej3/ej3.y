...
oplist:
    ...
    ;
l:
    /*vacio*/ {fprintf(fp,"R2: L -> lista_vacia \n");
                listaPtr = crearHoja("lista_vacia");
                }
    |exp {
        listaPtr = expPtr;
        fprintf(fp,"R3: L -> EXP \n");
...
exp:
    exp SUMA term  {
                    ...
                    fprintf(fp,"R7: EXP -> EXP + TERM \n"); 
                    }
    | term  {
            expPtr = termPtr;
            fprintf(fp,"R8: EXP -> TERM \n");
            }
    ;
term:
    term MULT factor {
                    termPtr = crearNodo("*", termPtr, factPtr);
                    fprintf(fp,"R9: TERM -> TERM * FACTOR \n");
                    }
    | factor {
            termPtr = factPtr;
            fprintf(fp,"R10: TERM -> FACTOR \n");
            }
    ;
factor:
    ID {
        factPtr = crearHoja($1);
        fprintf(fp,"R11: FACTOR -> id (%s) \n", yylval.valor_string);
        }
    | CTE {
        sprintf(str_aux, "%d",$1); 
        factPtr = crearHoja(str_aux);
        fprintf(fp,"R12: FACTOR -> cte (%d) \n", yylval.valor_int);
        }
    ;
%%
